...
module camera_global_config #(
    ...
    reg [$clog2(DATA_WIDTH)-1:0] x_shamt_reg;
    reg [$clog2(DATA_WIDTH)-1:0] y_shamt_reg;
    reg [$clog2(DATA_WIDTH)-1:0] a_shamt_reg;
    reg                          x_shiftd_reg;
    reg                          y_shiftd_reg;
    reg                          a_shiftd_reg;
    reg [        DATA_WIDTH-1:0] x_offset_reg;
    reg [        DATA_WIDTH-1:0] y_offset_reg;
    reg [        DATA_WIDTH-1:0] a_offset_reg;

    wire [DATA_WIDTH-1:0] conf = {a_shiftd_reg, y_shiftd_reg, x_shiftd_reg, a_shamt_reg, y_shamt_reg, x_shamt_reg, a_channel_reg, y_channel_reg, x_channel_reg};

    assign memory = {a_offset_reg, y_offset_reg, x_offset_reg, conf};
    ...
    assign x_shiftd  = x_shiftd_reg;
    assign y_shiftd  = y_shiftd_reg;
    assign a_shiftd  = a_shiftd_reg;
    ...
    always @(posedge clk) begin
        if (write[0]) begin
            ...
            x_shamt_reg   <= write_data[6+$clog2(DATA_WIDTH)*0+:$clog2(DATA_WIDTH)];
            y_shamt_reg   <= write_data[6+$clog2(DATA_WIDTH)*1+:$clog2(DATA_WIDTH)];
            a_shamt_reg   <= write_data[6+$clog2(DATA_WIDTH)*2+:$clog2(DATA_WIDTH)];
            x_shiftd_reg  <= write_data[6+$clog2(DATA_WIDTH)*3+0];
            y_shiftd_reg  <= write_data[6+$clog2(DATA_WIDTH)*3+1];
            a_shiftd_reg  <= write_data[6+$clog2(DATA_WIDTH)*3+2];
        end
        if (write[1]) begin
