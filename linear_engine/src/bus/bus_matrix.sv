...
`define BUS_MATRIX__TERMINATOR(source) \
    bus_terminator bus_terminator_``source`` ( \
        .in(source)                            \
    );                                         \

`define BUS_MATRIX__CONNECTOR1(slave, master) \
    bus_connector bus_connector_``master`` ( \
        .in (slave),                         \
        .out(master)                         \
    );                                       \

`define BUS_MATRIX__CONNECTOR2(slave1, slave2, master) \
    bus_arbiter bus_arbiter_``slave2`` ( \
        .clk   (clk),                    \
        .rst   (rst),                    \
        .port_a(slave1),                 \
        .port_b(slave2),                 \
...
`define BUS_MATRIX__CONNECTOR3(slave1, slave2, slave3, master) \
    ...
    bus_arbiter bus_arbiter_``slave3`` (     \
        .clk   (clk),                        \
        .rst   (rst),                        \
