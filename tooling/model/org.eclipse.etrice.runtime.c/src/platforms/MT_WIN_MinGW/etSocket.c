...
int etSocket_construct(etSocket* self, const char* address, etUInt16 port) {
...
int etSocket_recv(etSocket* self, uint8* buffer, size_t length) {
	int result = recv(self->socket, (char*) buffer, (int)length, 0);
	if(result < 0) {
...
int etSocket_send(etSocket* self, uint8* buffer, size_t length) {
	int result = send(self->socket, (const char*) buffer, (int)length, 0);
	if(result < 0) {
...
int etServerSocket_construct(etServerSocket* self, uint16_t port) {
	int result = initializeWinsock();
	if (result != 0) {
		return result;
	}
