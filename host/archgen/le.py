...
RD            = InstructionField("rd",             5, InstructionFieldType.INDEX,     INDEX_REG                )
...
# Jams
RD_ZERO  = InstructionFieldSpecialization("zero", 0, RD,  jam=True) # For Y format that repurposes rd but fixes it to zero
RS1_ZERO = InstructionFieldSpecialization("zero", 0, RS1, jam=True) # For Y format that repurposes rs1 but fixes it to zero

# Instruction Formats
R = InstructionFormat("r", (OPCODE_PREFIX, OPCODE, RD,                           FUNCT3, RS1,          RS2,           FUNCT7             )) # Register
I = InstructionFormat("i", (OPCODE_PREFIX, OPCODE, RD,                           FUNCT3, RS1,          (SIMM, 0, 11)                     )) # Immediate
H = InstructionFormat("h", (OPCODE_PREFIX, OPCODE, RD,                           FUNCT3, RS1,          (ZIMM, 0,  4), FUNCT7             )) # Immediate shift          (I specialization for immediate shifts)
Y = InstructionFormat("y", (OPCODE_PREFIX, OPCODE, RD_ZERO,                      FUNCT3, RS1_ZERO,     SYS                               )) # System                   (I specialization for system instructions, e.g., ECALL, EBREAK)
C = InstructionFormat("c", (OPCODE_PREFIX, OPCODE, RD,                           FUNCT3, RS1,          CSR                               )) # CSR                      (I specialization for CSR instructions)
D = InstructionFormat("d", (OPCODE_PREFIX, OPCODE, RD,                           FUNCT3, (ZIMM, 0, 4), CSR                               )) # CSR immediate            (I specialization for CSR instructions with immediate)
...
U = InstructionFormat("u", (OPCODE_PREFIX, OPCODE, RD, (SIMM, 12, 31)                                                                    )) # Upper immediate
J = InstructionFormat("j", (OPCODE_PREFIX, OPCODE, RD, (SIMM, 12, 19), (SIMM, 11, 11), (SIMM, 1, 10), (SIMM, 20, 20)                     )) # Jump
...
FUNCT3_CC     = InstructionFieldSpecialization("cc",     0b010, FUNCT3) # For Zicc, multiple instructions (CCEQ, CCNE, CCLT, CCGE, CCLTU, CCGEU)
FUNCT3_CCI    = InstructionFieldSpecialization("cci",    0b011, FUNCT3) # For Zicc, multiple instructions (CCEQI, CCNEI, CCLTI, CCGEI, CCLTUI, CCGEUI)
...
FUNCT7_M      = InstructionFieldSpecialization("m",      0b0000001, FUNCT7) # For M, multiple instructions (MUL, MULH, MULHSU, MULHU, DIV, DIVU, REM, REMU)

## CMP
### OPCODE BRANCH
CMP_EQ        = InstructionFieldSpecialization("eq",     0b00000, CMP) # For Zicc, multiple instructions (CCEQ, CCEQI)
CMP_NE        = InstructionFieldSpecialization("ne",     0b00001, CMP) # For Zicc, multiple instructions (CCNE, CCNEI)
CMP_LT        = InstructionFieldSpecialization("lt",     0b00100, CMP) # For Zicc, multiple instructions (CCLT, CCLTI)
CMP_GE        = InstructionFieldSpecialization("ge",     0b00101, CMP) # For Zicc, multiple instructions (CCGE, CCGEI)
CMP_LTU       = InstructionFieldSpecialization("ltu",    0b00110, CMP) # For Zicc, multiple instructions (CCLTU, CCLTUI)
CMP_GEU       = InstructionFieldSpecialization("geu",    0b00111, CMP) # For Zicc, multiple instructions (CCGEU, CCGEUI)
...
LW     = Instruction("lw",     I, (OPCODE_LOAD,            FUNCT3_W                   ), "R[rd] = zeroext(M[R[rs1] + signext(simm & 0x0000_0FFF)] & 0xFFFF_FFFF)")
...
X0  = IndexEntry("x0",   0, INDEX_REG, "zero", desc="Hard-wired zero"     )
X1  = IndexEntry("x1",   1, INDEX_REG, "ra",   desc="Return address"      )
X2  = IndexEntry("x2",   2, INDEX_REG, "sp",   desc="Stack pointer"       )
X3  = IndexEntry("x3",   3, INDEX_REG, "gp",   desc="Global pointer"      )
X4  = IndexEntry("x4",   4, INDEX_REG, "tp",   desc="Thread pointer"      )
X5  = IndexEntry("x5",   5, INDEX_REG, "t0",   desc="Temporary register 0")
X6  = IndexEntry("x6",   6, INDEX_REG, "t1",   desc="Temporary register 1")
X7  = IndexEntry("x7",   7, INDEX_REG, "t2",   desc="Temporary register 2")
X8  = IndexEntry("x8",   8, INDEX_REG, "s0",   desc="Saved register 0"    )
X9  = IndexEntry("x9",   9, INDEX_REG, "s1",   desc="Saved register 1"    )
...
X26 = IndexEntry("x26", 26, INDEX_REG, "s10",  desc="Saved register 10"   )
X27 = IndexEntry("x27", 27, INDEX_REG, "s11",  desc="Saved register 11"   )
